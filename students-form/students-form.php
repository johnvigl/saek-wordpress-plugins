...
// --- Αποθήκευση Ρυθμίσεων ---
        if ( isset($_POST['sf_save_settings']) ) {
            ...
            // --- Ρυθμίσεις Σύνδεσης με saek-hub ---
            update_option('sf_hub_url', esc_url_raw($_POST['sf_hub_url']));
            update_option('sf_hub_api_token', sanitize_text_field($_POST['sf_hub_api_token']));
            update_option('sf_hub_oauth_user', sanitize_text_field($_POST['sf_hub_oauth_user']));
            update_option('sf_hub_oauth_pass', sanitize_text_field($_POST['sf_hub_oauth_pass']));
        ...
        $hub_url        = get_option('sf_hub_url', '');
        $hub_api_token  = get_option('sf_hub_api_token', '');
        $hub_oauth_user = get_option('sf_hub_oauth_user', '');
        $hub_oauth_pass = get_option('sf_hub_oauth_pass', '');
        ?>
...
<p style="margin: 0 0 15px 0; font-size: 12px; color: #666;">Μπορείτε να εισάγετε πολλαπλά email διαχωρισμένα με κόμμα (,)</p>
                    ...
                    <?php
    $hub_settings_saved = !empty($hub_url) || !empty($hub_api_token) || !empty($hub_oauth_user) || !empty($hub_oauth_pass);
    ?>
<hr style="margin: 25px 0; border: 0; border-top: 1px solid #eee;">

<h3 style="margin-top: 0;"><?php echo $hub_settings_saved ? '✅ ' : ''; ?>Σύνδεση με saek-hub</h3>

<label><strong>Hub Base URL:</strong></label>
<input type="url" name="sf_hub_url" value="<?php echo esc_attr($hub_url); ?>" style="width: 100%; margin-bottom: 10px;" placeholder="https://saek-hub.example.gr">

<label><strong>Hub X-API-Key:</strong></label>
<input type="password" name="sf_hub_api_token" value="<?php echo esc_attr($hub_api_token); ?>" style="width: 100%; margin-bottom: 10px;">

<div style="display: flex; gap: 15px; margin-bottom: 20px;">
    <div style="flex: 1;">
        <label><strong>Hub OAuth Username:</strong></label>
        <input type="text" name="sf_hub_oauth_user" value="<?php echo esc_attr($hub_oauth_user); ?>" style="width: 100%;">
    </div>
    <div style="flex: 1;">
        <label><strong>Hub OAuth Password:</strong></label>
        <input type="password" name="sf_hub_oauth_pass" value="<?php echo esc_attr($hub_oauth_pass); ?>" style="width: 100%;">
    </div>
</div>
<p style="margin: 0 0 15px 0; font-size: 12px; color: #666;">Χρησιμοποιείται για αυτόματη αποστολή της αίτησης στο saek-hub μετά την υποβολή και την έκδοση πρωτοκόλλου.</p>
	...
	public function render_form() {
			...
			if ( empty( $errors ) ) {
				...
				$response = wp_remote_post( $api_url, array(
					...
					'body'        => wp_json_encode(array(
						...
						'comment'       => $comment,
						'in_perilipsi'  => $in_perilipsi_text,
						'out_perilipsi' => $out_perilipsi_text
					))
				));
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( $status_code === 200 ) {
						...
						if (isset($body->protocol_number)) {
							...
							$raw_protocol_number = strval($p_num);
				...
				// ==========================================
				// ΕΜΦΑΝΙΣΗ ΜΗΝΥΜΑΤΟΣ ΕΠΙΤΥΧΙΑΣ (πριν το SAEK-Hub)
				// ==========================================
				...
				if ( isset( $api_data['status'] ) && $api_data['status'] === 'success' ) {
					...
					echo "Η αίτησή σας υποβλήθηκε επιτυχώς!<br><br>";
				}
				...
				// ==========================================
				// SAEK-HUB API CALL (παρασκηνιακά, μετά το μήνυμα)
				// ==========================================
				$hub_url        = get_option('sf_hub_url', '');
				$hub_api_token  = get_option('sf_hub_api_token', '');
				$hub_oauth_user = get_option('sf_hub_oauth_user', '');
				...
				if ( ! empty( $hub_url ) && ! empty( $hub_api_token ) && ! empty( $raw_protocol_number ) ) {
					$hub_endpoint = trailingslashit( $hub_url ) . 'pegasus/certificate-by-email';
					$hub_response = wp_remote_post( $hub_endpoint, array(
						'method'      => 'POST',
						'timeout'     => 30,
						'sslverify'   => false,
						'headers'     => array(
							'Authorization' => 'Basic ' . base64_encode( $hub_oauth_user . ':' . $hub_oauth_pass ),
							'X-API-Key'     => $hub_api_token,
							'Content-Type'  => 'application/json',
						),
						'body'        => wp_json_encode(array(
							'email'    => $email,
							'protocol' => $raw_protocol_number,
						))
					));
					// Καταγραφή αποτελέσματος για debug
					if ( is_wp_error( $hub_response ) ) {
						error_log( 'SAEK-HUB ERROR: ' . $hub_response->get_error_message() );
					} else {
						$hub_status = wp_remote_retrieve_response_code( $hub_response );
						$hub_body = wp_remote_retrieve_body( $hub_response );
						error_log( 'SAEK-HUB RESPONSE: HTTP ' . $hub_status . ' - ' . $hub_body );
					}
				}
			} else {
		...
		<div class="cf-form-wrapper">
			...
			<form action="" method="post">
				<!-- Κρυφό πεδίο για να αναγνωρίζει η PHP την υποβολή ακόμα κι αν το κουμπί απενεργοποιηθεί -->
				...
				<div class="cf-form-group"><input type="submit" id="cf_submit_btn" value="Υποβολή" class="cf-submit-btn" disabled></div>
			</form>
		</div>
		<?php endif; ?>
		<script>
			...
			document.addEventListener('DOMContentLoaded', function() {
				...
				if (form) {
					...
					// Αλλαγή κουμπιού κατά την υποβολή
					form.addEventListener('submit', function() {
						if (submitBtn) {
							submitBtn.disabled = true;
							submitBtn.value = 'Αποστολή ⏳';
						}
					});
				}
